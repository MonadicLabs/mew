...
#include <atomic>
#include <cppbackports/optional.h>
#include <memory>
...
public:
    ...
    cpp17::optional<T> steal() {
        auto t = top.load(std::memory_order_acquire);
        std::atomic_thread_fence(std::memory_order_seq_cst);
        auto b = bottom.load(std::memory_order_acquire);

        int size = b - t;
        cpp17::optional<T> stolen = {};
...
public:
    explicit WorkerEndpoint(std::shared_ptr<Deque<T>> d) : deque(d) {
    }

    // Copy constructor.
    // There can only be one worker end.
    WorkerEndpoint(const WorkerEndpoint<T> &w) = delete;

    // Move constructor.
    WorkerEndpoint(WorkerEndpoint<T> &&w) : deque(std::move(w.deque)) {
    }
    ...
    cpp17::optional<T> pop() {
        return deque->pop_bottom();
    }
};

template <typename T>
class StealerEndpoint {
private:
    std::shared_ptr<Deque<T>> deque;
    buffer_tls *buffer_data;

public:
    explicit StealerEndpoint(std::shared_ptr<Deque<T>> d)
        : deque(d)
    ...
    StealerEndpoint(const StealerEndpoint<T> &s)
        : deque(s.deque)
        , buffer_data(deque->reclaimer.register_thread()) {
    }
    ...
    // thread.
    StealerEndpoint(StealerEndpoint<T> &&s)
...
template <typename T>
std::pair<WorkerEndpoint<T>, StealerEndpoint<T>> deque() {
                                                 auto d = std::make_shared<Deque<T>>();
                                                 return {WorkerEndpoint<T>(d), StealerEndpoint<T>(d)};
}
